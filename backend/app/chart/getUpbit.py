...
def update_coin_data():
    ...
    for res in tot_res:
        ...
        coin_object[sym]["price"] = pri
        coin_object[sym]["day_chage_price"] = res["signed_change_price"]
        coin_object[sym]["day_change_rate"] = res["signed_change_rate"]
        coin_object[sym]["highest_52_week_price"] = hi_pri
        coin_object[sym]["highest_52_week_rate"] = (
            round((hi_pri - pri) / hi_pri * 100, 2) * -1
        )
        coin_object[sym]["lowest_52_week_price"] = low_pri
        coin_object[sym]["lowest_52_week_rate"] = round(
...
def sort_by(base: str, reverse_flag: bool = None):
    if reverse_flag:
        coin_list.sort(key=lambda x: x[base], reverse=True)
    else:
        coin_list.sort(key=lambda x: x[base])

...
totURL_list = [
    "http://finlife.fss.or.kr/finlifeapi/depositProductsSearch.json?auth=fac934f307d1d0770ec3143a5f14eaa7&topFinGrpNo=020000&pageNo=1",
...
def getMortgage():
    ...
    for result in xml_obj.find_all("product"):
        ...
        for option in options:
            if (
                option.find("rpay_type_nm").text == "분할상환방식"
                and option.find("lend_rate_type_nm").text == "변동금리"
            ):
...
def getRentHouse():
    ...
    for result in xml_obj.find_all("product"):
        ...
        for option in options:
            if (
                option.find("rpay_type_nm").text == "만기일시상환방식"
                and option.find("lend_rate_type_nm").text == "변동금리"
            ):
